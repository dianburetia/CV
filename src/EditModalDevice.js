...
function EditModalDevice({ isOpenDevice, onRequestCloseDevice, onSave, dataDevice }) {
  ...
  useEffect(() => {
    axios.get('http://localhost:8091/accounts/accounts')
      .then((response) => {
  ...
  const handleEdit1 = () => {
    axios.put(`http://localhost:8090/devices/${dataDevice.id}`,{descriptions:editedData1.descriptions,
  ...
  const handleSave1 = () => {
    ...
    axios.post('http://localhost:8090/devices',{
