...
function EditModal({ isOpen, onRequestClose, onSave, data }) {
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:8090/devices/devicess')
      .then((response) => {
  ...
  const handleSave1 = () => {
   ...
   console.log(idsArray)
    axios.post('http://localhost:8091/accounts',{name:editedData.name,
  ...
  const handleEdit = () => {
    ...
    axios.put(`http://localhost:8091/accounts/${data.id}`,{name:editedData.name,
    role:editedData.role,
    password:editedData.password,
    devicesIds:idsArray})
      .then((response) => {
