...
function AdminPage() {
  ...
  useEffect(() => {
    axios.get('http://localhost:8091/accounts')
      ...
      axios.get('http://localhost:8090/devices')
      .then((response) => {
  ...
  const handleDelete = (index) => {
    ...
    axios.delete(`http://localhost:8091/accounts/${data[index].id}`)
    .then((response) => {
      setData(response.data);
    })
    .catch((error) => {
  ...
  const handleDeleteDevice = (indexDevice) => {
    setEditedRowDevice(dataDevice[indexDevice]);
    axios.delete(`http://localhost:8090/devices/${dataDevice[indexDevice].id}`)
    .then((response) => {
      setData(response.data);
    })
    .catch((error) => {
